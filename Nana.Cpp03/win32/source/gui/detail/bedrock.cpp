...
#pragma pack()
	...
	bedrock::thread_context* bedrock::open_thread_context(unsigned tid)
	{
		...
		thread_context * context = (i == impl_->tcontexts.end() ? &(impl_->tcontexts[tid]) : &(i->second));
	...
	void bedrock::map_thread_root_buffer(bedrock::core_window_t* wd)
	{
		::PostMessage(reinterpret_cast<HWND>(wd->root), nana::detail::messages::map_thread_root_buffer, reinterpret_cast<WPARAM>(wd), 0);
	}

	void interior_helper_for_menu(MSG& msg, nana::gui::native_window_type menu_window)
	{
		switch(msg.message)
		{
		case WM_KEYDOWN:
	...
	void make_eventinfo(nana::gui::eventinfo& ei, nana::gui::detail::bedrock::core_window_t* wnd, unsigned msg, const parameter_decoder& pmdec)
	{
	...
	bool trivial_message(HWND wd, UINT msg, WPARAM wParam, LPARAM lParam, LRESULT & ret)
	{
		...
		case nana::detail::messages::remote_thread_destroy_window:
			...
			{
				nana::threads::scope_guard sg(bedrock.wd_manager.internal_lock());
	...
	LRESULT CALLBACK Bedrock_WIN32_WindowProc(HWND root_window, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		nana::gui::native_window_type native_window = reinterpret_cast<nana::gui::native_window_type>(root_window);
		...
		if(root_runtime)
		{
			...
			case WM_MOUSEMOVE:
				...
				if(mousemove_window && (msgwnd != mousemove_window))
				{
					...
					make_eventinfo(ei, mousemove_window, message, pmdec);
	...
	bool bedrock::raise_event(unsigned eventid, core_window_t* wd, const eventinfo& ei, bool ask_update)
	{
		...
		bedrock_object.evt_manager.answer(eventid, reinterpret_cast<window>(wd), ei, event_manager::event_kind::both);
	...
	bool bedrock::raise_event_keep_ei(unsigned eventid, core_window_t* wd, const eventinfo &ei, bedrock::thread_context* thrd)
	{
		if(bedrock_object.wd_manager.available(wd) == false) return false;
