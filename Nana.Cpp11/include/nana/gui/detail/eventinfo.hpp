...
namespace gui
{
	...
	namespace detail
	{
		struct event_tag
		{
			...
			inline static bool accept(unsigned event_id, category::flags categ)
			{
				return (event_id < event_tag::count) && ((static_cast<int>(event_category[event_id]) & static_cast<int>(categ)) == static_cast<int>(event_category[event_id]));
			}

			static category::flags event_category[event_tag::count];
		};
