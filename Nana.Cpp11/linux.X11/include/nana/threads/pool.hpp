	...
	namespace threads
	{
		...
		public:
			...
			void push(const Function& f) volatile
			{
				...
				try
				{
					tkob = new detail::task<typename nana::metacomp::static_if<std::is_function<Function>::value, Function*, Function>::value_type>(f);
					_m_push(tkob);
				}
				catch(std::bad_alloc&)
				{
		...
		private:
			...
			struct locks_tag
			{
				std::recursive_mutex thread;
				std::recursive_mutex task;
				std::mutex signal;
