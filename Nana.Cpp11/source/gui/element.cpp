...
namespace nana{	namespace gui
{
	...
	namespace element
	{
		...
		{
			bool draw(graph_reference graph, nana::color_t bgcolor, nana::color_t fgcolor, const nana::rectangle& r, element_state es, state crook_state) override
			{
		...
		public:
			bool draw(graph_reference graph, nana::color_t bgcolor, nana::color_t fgcolor, const nana::rectangle& r, element_state es, state crook_state) override
			{
				...
				case element_state::focus_normal:
					colormap = (crook_state != state::unchecked ? &bmp_checked : &bmp_unchecked);
					break;
				case element_state::hovered:
				case element_state::focus_hovered:
					colormap = (crook_state != state::unchecked ? &bmp_checked_highlight : &bmp_unchecked_highlight);
					break;
				case element_state::pressed:
		...
		: nana::noncopyable, nana::nonmovable
	{
		...
		element_manager()
		{
			...
			element::add_crook<element::crook_radio>("radio");
		}
	...
	//template<> class facade<element::crook>
		facade<element::crook>::facade()
			:	state_(facade<element::crook>::state::unchecked),
		...
		{
			state_ = (state_ == facade<element::crook>::state::unchecked ? facade<element::crook>::state::checked : facade<element::crook>::state::unchecked);
			return *this;
		}
		...
		{
			return (*keeper_)->draw(graph, bgcol, fgcol, r, es, state_);
		}
