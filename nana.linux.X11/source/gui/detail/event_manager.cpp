...
namespace detail
{
	namespace inner_event_manager
	{
		 ...
		 *		The definition of struct inner_handler_invoker
		 */
		struct inner_handler_invoker
		{
		public:
		...
		struct handler_queue
		{
			...
			void copy(Container& container)
			{
				...
				size_t begin = size_;
				allocate(size);
				abstract_handler * *i = queue_ + begin;
	...
	//class event_manager
		...
		bool event_manager::answer(unsigned eventid, window wd, const eventinfo& ei, event_kind::t evtkind)
		{
		...
		//@param:wnd, the triggering window
		...
		event_handle event_manager::_m_make(unsigned eventid, window wnd, abstract_handler* abs_handler, bool drawer_handler, window listener)
		{
			if(abs_handler == 0)	return 0;

			abs_handler->window = wnd;
			...
			
			{
				...
				abs_handler->container = (drawer_handler ?
												&(nana_runtime::callbacks.table[eventid][wnd].first) :
												&(nana_runtime::callbacks.table[eventid][wnd].second));
			...
			if(drawer_handler == false)
				bedrock::instance().wd_manager.event_filter(reinterpret_cast<bedrock::core_window_t*>(wnd), true, eventid);
			//call the event_register
			nana::detail::platform_spec::instance().event_register_filter(
					bedrock::instance().root(reinterpret_cast<bedrock::core_window_t*>(wnd)),
					eventid);
