		...
		void trigger::_m_draw_title(graph_reference graph, bool enabled)
		{
			...
			if(ts.width)
			{
				if(enabled)
				{
					...
					{
						nana::size ts;
						graph.string(x, y, fgcolor, str, omitted_pixels, true, ts);
					}
					else
				...
				else
				{
					...
					{
						nana::size ts;
						graph.string(x + 1, y + 1, 0xFFFFFF, str, omitted_pixels, true, ts);
						graph.string(x, y, 0x808080, str, omitted_pixels, true, ts);
					}
					else
					{
