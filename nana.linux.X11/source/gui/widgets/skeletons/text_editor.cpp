	...
	namespace skeletons
	{
		...
		nana::string text_editor::text() const
		{
			...
			if(lines > 0)
			{
				...
				for(size_t i = 1; i < textbase_.lines(); ++i)
				{
					str += STR("\n\r");
	...
	//private:
		bool text_editor::_m_scroll_text(bool vertical)
		{
			if(attributes_.vscroll && vertical)
			{
				...
				{
					points_.offset.y = static_cast<int>(attributes_.vscroll->value());
					return true;
				}
			}
		...
		//_m_get_coordinate
		//@brief: Get the coordinate of a string with a speific length
		nana::point text_editor::_m_get_coordinate(const char_type* text, unsigned n, unsigned line_height) const
		{
			nana::point pos;
			pos.x = static_cast<int>(_m_text_extent_size(text, n).width) + text_area_.area.x - points_.offset.x;
			pos.y = (points_.caret.y - points_.offset.y) * line_height + text_area_.area.y;
			return pos;
		}
			...
			for(std::vector<unicode_bidi::entity>::iterator i = reordered.begin(); i != reordered.end(); ++i)
			{
				if(i->begin <= ch && ch <= i->end)
				{
					if(_m_is_right_text(*i))
					{
						//RTL
						size_t len = i->end - i->begin;
						unsigned w = _m_text_extent_size(i->begin, len).width;
						text_w += w - graph_.glyph_extent_size(i->begin, len, 0, ch - i->begin).width;
					}
					else
					{
						//LTR
					...
					text_w += _m_text_extent_size(i->begin, i->end - i->begin).width;
			}
			return text_w;
		}
