...
namespace nana
{
		...
		bool date::operator <(const date & r) const
		{
			//return (value_.year < r.value_.year || (value_.year == r.value_.year ? (value_.month < r.value_.month || (value_.month == r.value_.month ? (value_.day < r.value_.month) : false)) : false ));
