	...
	{}
		//class drawer
			...
			void drawer::_m_draw(drawer::graph_reference graph)
			{
				...
				unsigned first_line_height = _m_draw_title(graph);
				_m_draw_checkbox(graph, first_line_height);
				API::lazy_refresh();
			}
			...
			unsigned drawer::_m_draw_title(nana::paint::graphics& graph)
			{
				nana::string title = widget_->caption();
				unsigned fgcolor = widget_->foreground();
				unsigned ts = graph.text_extent_size(STR("jhAP{[|("), 8).height;
				...
				if(graph.width() > 16 + interval)
				{
					...
					{
						//graph.string( 16 + interval + 1, 2, 0xFFFFFF, title);
						graph.string(17 + interval, 2, 0xFFFFFF, title, pixels, lines);
						fgcolor = 0x808080;
					}

					//graph.string( 16 + interval, 1, fgcolor, title);
					graph.string(16 + interval, 1, fgcolor, title, pixels, lines);
				}
				return ts + 2;
			}
		//end class drawer
